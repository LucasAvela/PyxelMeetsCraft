...
class Debug:
    ...
    def UpdateDebug():
        ...
        if pyxel.btnp(pyxel.KEY_EQUALS):
            for key, item in Optfine.blocksToRenderer.items():
                if item['Block'] != Data.Blocks.Air.value:
                    print(key, item)
        ...
        if pyxel.btnp(pyxel.MOUSE_BUTTON_LEFT):
            ...
            while layer > 1:
                ...
                if (target_x, adj_y, layer) in WorldGen.World and \
            ...
            if (target_x, adj_y, layer) in WorldGen.World:
                if ((target_x, adj_y, layer + 1) not in WorldGen.World or 
                    WorldGen.World[(target_x, adj_y, layer + 1)]['Block'] == Data.Blocks.Air.value):
                    WorldGen.World[(target_x, adj_y, layer)]['Block'] = Data.Blocks.Air.value
            ...
            if (target_x, adj_y, layer) in WorldGen.World:
                if ((target_x, adj_y, layer + 1) not in WorldGen.World or 
                    ...
                    if (layer + 1) < Data.layers_quantity:
                        # Acessa diretamente os dados do bloco pelo dicionário
                        block = Data.block_data[Debug.blocks_list[Debug.selected_block]]
                        WorldGen.World[(target_x, adj_y, layer + 1)]['Block'] = Debug.blocks_list[Debug.selected_block]
                        WorldGen.World[(target_x, adj_y, layer + 1)]['Solid'] = block['Solid']
    ...
    def DrawDebug():
        ...
        if (target_x, adj_y, layer) in WorldGen.World:
            if ((target_x, adj_y, layer + 1) not in WorldGen.World or 
                WorldGen.World[(target_x, adj_y, layer + 1)]['Block'] == Data.Blocks.Air.value):
                pyxel.blt(target_x * Data.block_Size,
                          (target_y * Data.block_Size) - ((layer % 2) * Data.block_Height),
                          1, 48, 240, Data.block_Size, Data.block_Size, 2)
        ...
        if (target_x, adj_y, layer) in WorldGen.World:
            if ((target_x, adj_y, layer + 1) not in WorldGen.World or 
                WorldGen.World[(target_x, adj_y, layer + 1)]['Block'] == Data.Blocks.Air.value):
                block_name = WorldGen.World[(target_x, adj_y, layer)]['Block']
                pyxel.text(4, 12, f'{block_name}', 7)
